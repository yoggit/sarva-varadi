...
async function main() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    runCLI(writeTmp(selData), path.join(ROOT, 'demo-selenium/sarva-report'), 'testng-selenium');
    process.stdout.write('selenium ✓  ');

    // REST Assured TestNG
    const raData = applyRestAssuredScenario(raSrc, RA[i], base, [4, 8]);
    runCLI(writeTmp(raData), path.join(ROOT, 'demo-restassured/sarva-report'), 'testng-listener');
    process.stdout.write('rest-assured ✓  ');

    // REST Assured JUnit5
    const juData = applyRestAssuredScenario(juSrc, JU[i], base, []);
    runCLI(writeTmp(juData), path.join(ROOT, 'demo-restassured-junit/sarva-report'), 'testng-listener');
